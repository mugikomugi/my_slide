$(function () {
  ...
  class Slide {
    ...
    prevClick() {
      this.prev.off('click').on('click', () => {
        ...
        this.prevSlide();
        if (this.autoPlay === true) {
          this.timerID = setTimeout(() => { this.slideStart(); }, slideEl.wait);
        }
      });
    }
  ...
  function move_check(e) {
    ...
    if (posiX - getX(e) > swipeThreshold) // swipeThreshold以上移動でスワイプと判断
    {
      /* 右→左と判断 */
      moveX = 'left';
    } else if (posiX - getX(e) < -swipeThreshold) // swipeThreshold以上移動でスワイプと判断
    {
      /* 左→右と判断 */
      moveX = 'right';
    }
  ...
  function end_check(e) {
    ...
    if (slideEl.autoPlay === true) {
      slideEl.timerID = setTimeout(() => { slide.slideStart(); }, slideEl.wait);
    }
  }
