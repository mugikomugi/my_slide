  ...
  class Slide {
    ...
    slideW() {
      if (window.innerWidth > 768) {
        this.imgW = $('picture', this.panelImg).width();
      } else {
    ...
    nextSlide() {
      ...
      if (this.click === true) {
        ...
        //画像クローン
        cloneImg = nextImg.children('picture').clone();
        this.panelImg.append(cloneImg);
        //actの付替え
        nextImg.addClass(this.act).siblings().removeClass(this.act);
        //スライド
        this.panelImg.stop().animate({ 'margin-left': this.imgW * -1 }, slideEl.wait,
          () => {
            $('picture', this.panelImg).first().remove();
    ...
    prevSlide() {
      ...
      if (this.click === true) {
        ...
        //画像クローン
        cloneImg = prevImg.children('picture').clone();
        this.panelImg.prepend(cloneImg).css('margin-left', this.imgW * -1);
        prevImg.addClass(this.act).siblings().removeClass(this.act);
        //スライド
        this.panelImg.stop().animate({ 'margin-left': 0 }, slideEl.wait,
          () => {
            $('picture', this.panelImg).last().remove();
            //スライドが終わったらクリックOK
            this.click = true;
          });
      }
    ...
    prevClick() {
      this.prev.on('click', () => {
        ...
        this.prevSlide();
        if (this.autoPlay === true) {
          timerID = setTimeout(() => { this.slideStart(); }, slideEl.speed);
        }
      });
    }
    ...
    clickThumb() {
      this.thumb.on('click', (e) => {
        ...
        if (this.click === true) {
          ...
          $('picture img', this.panelImg).attr('src', $('img', imgNum).attr('src'));
          if (this.autoPlay === true) {
            timerID = setTimeout(() => { this.slideStart(); }, slideEl.speed);
          }
          this.click = true;
